...
- **Formally**:  
  \[
  f: \Theta \to \mathcal{X} \times \mathbb{R}^n
  \]
  where:
  - \(\Theta\) = type space
  - \(\mathcal{X}\) = outcome space
  - \(\mathbb{R}^n\) = transfers (payments)
...
- **Two Rules**:
  1. **Allocation Rule**: Choose outcome that maximizes total reported welfare.
     \[
     x^* = \arg\max_{x \in X} \sum \tilde{v}_i(x)
     \]
  2. **Payment Rule (Clarke Pivot Rule)**:
     \[
     t_i = \left[\max_{x} \sum_{j \neq i} \tilde{v}_j(x)\right] - \left[\sum_{j \neq i} \tilde{v}_j(x^*)\right]
     \]
